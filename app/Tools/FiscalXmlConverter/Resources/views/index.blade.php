...
>
    ...
    @elseif ($result)
        <div class="d-flex flex-wrap gap-2 mb-3">@foreach(['pdf'=>'PDF','xlsx'=>'Excel (.xlsx)','csv'=>'CSV','json'=>'JSON'] as $format=>$label)<a class="btn btn-sm btn-outline-secondary" href="{{ route('tools.conversor-fiscal-xml.export', ['format' => $format, 'result_token' => $currentResultToken ?? '']) }}" @if(in_array($format, ['pdf', 'xlsx', 'csv'], true)) data-testid="download-{{ $format }}" @endif data-analytics-action="export" data-analytics-form="single" data-analytics-format="{{ $format }}">Exportar {{ $label }}</a>@endforeach</div>
    @endif
