...
Route::prefix('ferramentas/conversor-fiscal-xml')->name('tools.conversor-fiscal-xml.')->group(function (): void {
    ...
    Route::get('/exportar/{format}', [ToolController::class, 'exportCurrent'])->whereIn('format', ['csv', 'json', 'pdf', 'xlsx'])->middleware('tool.feature:conversor-fiscal-xml,professional_export')->name('export');
    Route::middleware(['tool.feature:conversor-fiscal-xml,history', 'persistence.auth'])->prefix('historico')->name('history.')->group(function (): void {
        ...
        Route::get('/{run}/exportar/{format}', [ToolController::class, 'exportHistory'])->whereIn('format', ['csv', 'json', 'pdf', 'xlsx'])->middleware('tool.feature:conversor-fiscal-xml,professional_export')->name('export');
    });
});
