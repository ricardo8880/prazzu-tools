...
final class Calculator implements ToolCalculator
{
    ...
    public function calculate(ToolCalculationInput $input): ToolCalculationResult
    {
        if (! $input instanceof CalculationInput) {
            throw new InvalidArgumentException('Entrada incompatível com a calculadora de ICMS-ST.');
        }
        ...
        if (! in_array($input->operationType, ['internal', 'interstate'], true)) {
            throw new InvalidArgumentException('Tipo de operação inválido.');
        }
        ...
        $operationBase = Money::zero();
        foreach ($components as $component) {
            $operationBase = $operationBase->add($component);
        }
        $discount = Money::fromDecimal($input->discount);
        $operationBase = Money::fromMinor(max(0, $operationBase->minorAmount() - $discount->minorAmount()));
        if ($operationBase->minorAmount() <= 0) {
            throw new InvalidArgumentException('A base da operação deve ser maior que zero.');
        }
        ...
        foreach ($input->items as $item) {
            $value = Money::fromDecimal($item['merchandise_value']);
    ...
    private function adjustedMva(Percentage $original, Percentage $interstate, Percentage $internal): Percentage
    {
        ...
        if ($intra >= $scale) {
            throw new InvalidArgumentException('Alíquota interna inválida para ajuste da MVA.');
        }
