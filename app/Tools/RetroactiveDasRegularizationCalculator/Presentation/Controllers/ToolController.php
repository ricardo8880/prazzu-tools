...
final class ToolController extends Controller
{
    ...
    public function calculate(ExecuteToolRequest $request, CalculateTool $action, ToolFeatureRequestAuthorizer $features, Tool $module, ShowToolPage $page): View
    {
        ...
        $features->requireIf((int) ($data['regularization_months'] ?? 1) > 1, $module, 'regularization', $request);
        if (! $plusEnabled) {
            $data['competencies'] = [];
        ...
        return view('tools-calculadora-das-retroativo-regularizacao-simples::index', [...$page->execute(), 'result' => $result, 'calculationInput' => $data, 'plusEnabled' => $plusEnabled, 'regularizationAllowed' => $features->allows($module, 'regularization', $request)]);
    }
