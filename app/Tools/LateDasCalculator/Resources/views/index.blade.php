...
@isset($result)
    <span data-analytics-result="main" hidden></span><x-tools.result-panel title="Atualização"><div class="row g-3">@foreach($result->summary as $item)<div class="col-md-6"><x-tools.result-metric :label="$item->label" :value="$item->value" :description="$item->description"/></div>@endforeach</div><h3 class="h5 mt-4">Memória</h3><div class="table-responsive"><table class="table table-sm"><tbody>@foreach(($result->calculationMemory?->steps ?? []) as $step)<tr><th>{{ $step->label }}<div class="small fw-normal text-body-secondary">{{ $step->formula }}</div></th><td class="text-end">{{ is_int($step->result) ? 'R$ '.number_format($step->result / 100, 2, ',', '.') : $step->result }}</td></tr>@endforeach</tbody></table></div><div class="alert alert-warning">Estimativa. A emissão oficial deve ser feita no PGDAS-D/Portal do Simples, que aplica a Selic oficial.</div><x-tools.export-buttons :pdf-route="route('tools.das-em-atraso.export.pdf')" :excel-route="route('tools.das-em-atraso.export.excel')" :input="$calculationInput ?? []" />
            ...
            />
</x-tools.result-panel>@endisset</x-tools.page>
@endsection
