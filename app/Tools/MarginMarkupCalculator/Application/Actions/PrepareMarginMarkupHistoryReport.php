...
final readonly class PrepareMarginMarkupHistoryReport
{
 ...
 {
  $run=$this->ownedRun->execute($runId,$userId); $result=$run->result;
  if (($result['calculation_type'] ?? 'single') !== 'single') throw new HttpException(422,'O PDF está disponível para cálculos individuais.');
