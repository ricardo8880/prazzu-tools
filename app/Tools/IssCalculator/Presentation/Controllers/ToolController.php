...
final class ToolController extends Controller
{
    ...
    {
        return view('tools-calculadora-iss::index', [...$page->execute(), 'plusEnabled' => $features->plusEnabled($module, $request)]);
    }
    ...
    public function calculate(ExecuteToolRequest $request, CalculateTool $action, ToolFeatureRequestAuthorizer $features, Tool $module, ShowToolPage $page): View
    {
        ...
        $request->flash();
        return view('tools-calculadora-iss::index', [...$page->execute(), 'result' => $result, 'calculationInput' => $data, 'plusEnabled' => $features->plusEnabled($module, $request)]);
    }
    ...
    private function input(array $d): CalculationInput
    {
        ...
        foreach (($d['services'] ?? []) as $x) {
            if (trim((string) ($x['value'] ?? '')) === '') continue;
        ...
        foreach (($d['municipality_scenarios'] ?? []) as $x) {
            if (trim((string) ($x['municipality'] ?? '')) === '' || trim((string) ($x['rate'] ?? '')) === '') continue;
