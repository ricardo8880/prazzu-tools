...
final class Calculator implements ToolCalculator
{
    ...
    {
        if(!$input instanceof CalculationInput) throw new InvalidArgumentException('Entrada incompatível com a Calculadora de ISS.');
        if($input->services===[]) throw new InvalidArgumentException('Informe ao menos um serviço.');
        $rows=[]; $totalValue=0; $totalIss=0; $totalRetained=0; $municipal=[]; $monthly=[];
        foreach($input->services as $service){
            $value=Money::fromDecimal((string)$service['value']); $rate=Percentage::fromString((string)$service['rate']);
            if($value->minorAmount()<=0 || $rate->millionthsOfPercent()<0) throw new InvalidArgumentException('Valor e alíquota devem ser válidos.');
            $iss=$value->percentage($rate)->minorAmount(); $retained=(bool)($service['retained']??false);
            ...
            $rows[]=$row; $totalValue+=$value->minorAmount(); $totalIss+=$iss; if($retained)$totalRetained+=$iss;
            $key=$row['municipality']; if(!isset($municipal[$key]))$municipal[$key]=['municipality'=>$key,'services'=>0,'value_minor'=>0,'iss_minor'=>0]; $municipal[$key]['services']++; $municipal[$key]['value_minor']+=$value->minorAmount(); $municipal[$key]['iss_minor']+=$iss;
            $monthKey=$row['competence'].'|'.$row['municipality']; if(!isset($monthly[$monthKey]))$monthly[$monthKey]=['competence'=>$row['competence'],'municipality'=>$row['municipality'],'services'=>0,'value_minor'=>0,'iss_minor'=>0,'retained_minor'=>0,'payable_by_provider_minor'=>0]; $monthly[$monthKey]['services']++; $monthly[$monthKey]['value_minor']+=$value->minorAmount(); $monthly[$monthKey]['iss_minor']+=$iss; if($retained)$monthly[$monthKey]['retained_minor']+=$iss; else $monthly[$monthKey]['payable_by_provider_minor']+=$iss;
        }
        $main=$rows[0]; $scenarios=[];
        foreach($input->municipalityScenarios as $scenario){
            $rate=Percentage::fromString((string)$scenario['rate']); $iss=Money::fromMinor($main['value_minor'])->percentage($rate)->minorAmount();
        ...
        if((float)$main['rate']<2 || (float)$main['rate']>5)$warnings[]=new ToolCalculationWarning('rate_check','A alíquota informada está fora da faixa geral de 2% a 5% prevista nas normas nacionais do ISS. Confirme exceções e legislação municipal antes de usar o resultado.',ToolCalculationWarningLevel::Info);
