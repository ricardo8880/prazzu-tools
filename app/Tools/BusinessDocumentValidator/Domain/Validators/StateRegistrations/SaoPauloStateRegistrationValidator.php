...
{
    public function state(): string { return 'SP'; }
    public function label(): string { return 'São Paulo'; }

    public function validate(string $registration): bool
    {
        $ie = $this->digits($registration);
        if (strlen($ie) !== 12 || preg_match('/^(\d)\1+$/', $ie)) return false;

        $sum = 0;
        foreach ([1, 3, 4, 5, 6, 7, 8, 10] as $i => $weight) $sum += ((int) $ie[$i]) * $weight;
        $first = $sum % 11;
        if ($first === 10) $first = 0;
        if ($first !== (int) $ie[8]) return false;

        $sum = 0;
        foreach ([3, 2, 10, 9, 8, 7, 6, 5, 4, 3, 2] as $i => $weight) $sum += ((int) $ie[$i]) * $weight;
        $second = $sum % 11;
