...
<x-tools.page :title="$tool->name" :description="$tool->description" :icon="$tool->icon" :slug="$tool->slug">
    <x-tools.form-panel title="Folha e enquadramento">
        <form method="POST" action="{{ route('tools.inss-patronal.calculate') }}" class="row g-3">
            ...
            <div class="col-12 col-md-6"><x-tools.form.input name="adjusted_rat" label="RAT ajustado pelo FAP" type="number" step="0.000001" min="0" max="15" suffix="%" :value="old('adjusted_rat', '1')" required /></div>
            <div class="col-12 col-md-6"><x-tools.form.input name="third_parties" label="Terceiros" type="number" step="0.000001" min="0" max="15" suffix="%" :value="old('third_parties', '5.8')" required /></div>
