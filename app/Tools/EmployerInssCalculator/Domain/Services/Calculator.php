...
final class Calculator implements ToolCalculator
{
    ...
    public function calculate(ToolCalculationInput $input): ToolCalculationResult
    {
        ...
        $cpp = $input->payroll->percentage($rates->cpp); $rat = $input->payroll->percentage($rates->rat);
        $third = $input->payroll->percentage($rates->thirdParties); $total = $cpp->add($rat)->add($third);
