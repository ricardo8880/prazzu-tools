...
Route::prefix('ferramentas/calculadora-ferias')->name('tools.calculadora-ferias.')->group(function (): void {
    ...
    Route::post('/exportar/{format}', [ToolController::class, 'exportCurrent'])->whereIn('format', ['csv', 'json', 'pdf', 'xlsx'])->middleware('tool.feature:calculadora-ferias,professional_export')->name('export');
    Route::middleware(['tool.feature:calculadora-ferias,vacation_planning', 'tool.feature:calculadora-ferias,multiple_employees'])->group(function (): void {
    ...
    Route::middleware(['tool.feature:calculadora-ferias,history', 'persistence.auth'])->prefix('historico')->name('history.')->group(function (): void {
        ...
        Route::get('/{run}/exportar/{format}', [ToolController::class, 'exportHistory'])->whereIn('format', ['csv', 'json', 'pdf', 'xlsx'])->middleware('tool.feature:calculadora-ferias,professional_export')->name('export');
    });
});
