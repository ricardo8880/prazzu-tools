...
final class ToolController extends Controller {
 ...
 public function calculate(ExecuteToolRequest $request, Calculator $calculator, ToolRunRecorder $recorder, ToolPersistenceAuthorizer $persistence, Tool $module): View {
  ...
  try { $result=$calculator->calculate($input); if($run!==null)$recorder->succeed($run,$result->toPersistenceArray()); } catch(Throwable $e) { if($run!==null)$recorder->fail($run,'calculadora-hora-extra.calculation_failed'); throw $e; }
  $request->flash(); return view('tools-calculadora-hora-extra::index',['result'=>$result,'historySaved'=>$run!==null]);
 }
 public function exportCurrent(ExecuteToolRequest $request, Calculator $calculator, ToolResultExportFactory $documents, PdfExporter $pdf, SpreadsheetExporter $spreadsheet, string $format): Response {
  abort_unless(in_array($format,['pdf','xlsx'],true),404); $input=$this->input($request->validated()); $result=$calculator->calculate($input); $filename='hora-extra-'.now()->format('Y-m-d');
