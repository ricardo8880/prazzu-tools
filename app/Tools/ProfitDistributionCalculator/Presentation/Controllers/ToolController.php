...
final class ToolController extends Controller
{
    ...
    public function calculate(ExecuteToolRequest $request, Calculator $calculator): View
    {
        ...
        $result = $calculator->calculate(new CalculationInput(
            partnerLabel: $data['partner_label'] ?? 'Sócio',
            ownershipPercentage: $data['ownership_percentage'],
            accountingProfit: $data['accounting_profit'],
            accumulatedLosses: $data['accumulated_losses'] ?? '0',
            reservesAndUnavailableAmounts: $data['reserves_and_unavailable_amounts'] ?? '0',
            adjustments: $data['adjustments'] ?? '0',
            priorDistributions: $data['prior_distributions'] ?? '0',
            intendedDistribution: $data['intended_distribution'],
        ));
