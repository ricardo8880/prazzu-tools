...
use App\Tools\IncomeStatementGenerator\Tool;
use Illuminate\Contracts\Auth\Authenticatable;
use PHPUnit\Framework\TestCase;

final class PlusFeaturesTest extends TestCase
{
    #[CoversPlusFeature('declaracao-rendimentos', 'spreadsheet_export')]
    public function test_spreadsheet_export_blocks_free_and_allows_plus_in_monetized_mode(): void
    {
        $manifest = (new Tool)->manifest();
        $user = $this->createStub(Authenticatable::class);
        $free = $this->gate(SubscriptionPlan::Free)->decide($manifest, 'spreadsheet_export', $user);
        $plus = $this->gate(SubscriptionPlan::Plus)->decide($manifest, 'spreadsheet_export', $user);
