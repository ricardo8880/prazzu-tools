...
@isset($result)@php($d=$result->details['document'])<x-tools.result-panel title="Declaração de Rendimentos"><div class="card"><div class="card-body"><h3 class="text-center">{{$d['title']}}</h3><p>Declaramos que <strong>{{$d['beneficiary']}}</strong>, documento {{$d['document']}}, recebeu de {{$d['payer']}} no ano-calendário {{$d['year']}} rendimentos brutos de {{$d['gross']}}.</p><table class="table"><tr><th>INSS</th><td>{{$d['inss']}}</td></tr><tr><th>IRRF</th><td>{{$d['irrf']}}</td></tr><tr><th>Outras deduções</th><td>{{$d['other']}}</td></tr><tr><th>Líquido</th><td>{{$d['net']}}</td></tr></table></div></div><x-tools.print-button class="mt-3">Imprimir / salvar PDF</x-tools.print-button></x-tools.result-panel>@endisset</x-tools.page>
@endsection
