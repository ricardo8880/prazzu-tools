...
{
    #[CoversPlusFeature('simulador-mei-microempresa', 'annual_projection')]
    #[CoversPlusFeature('simulador-mei-microempresa', 'business_costs')]
    #[CoversPlusFeature('simulador-mei-microempresa', 'migration_point')]
    public function test_lot_five_features_block_free_and_allow_plus_in_monetized_mode(): void
    {
        $manifest = (new Tool)->manifest();
        $this->assertFeatureAccess($manifest, 'annual_projection');
        $this->assertFeatureAccess($manifest, 'business_costs');
        $this->assertFeatureAccess($manifest, 'migration_point');
        $this->assertFeatureAccess($manifest, 'report');
    }
