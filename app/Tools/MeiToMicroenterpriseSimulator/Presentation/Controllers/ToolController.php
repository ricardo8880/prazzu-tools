...
final class ToolController extends Controller
{
    ...
    {
        $data = $request->validated(); $plusEnabled = $features->plusEnabled($module, $request);
        if (! $plusEnabled) {
    ...
    {
        abort_unless(in_array($format, ['pdf', 'xlsx'], true), 404); $input = $this->input($request->validated()); $result = $action->execute($input); $filename = 'simulacao-mei-microempresa-'.now()->format('Y-m-d');
