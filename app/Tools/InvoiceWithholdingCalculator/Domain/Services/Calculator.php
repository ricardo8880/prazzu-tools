...
final class Calculator implements ToolCalculator
{
    ...
    {
        if (! $input instanceof CalculationInput) throw new InvalidArgumentException('Entrada incompatível com a ferramenta calculadora-retencoes-nota-fiscal.');
        ...
        foreach($input->notes as $note){
            ...
            if($value->minorAmount()<=0) continue;
            $notes[]=['description'=>$note['description'] !== '' ? $note['description'] : 'Nota/serviço adicional','invoice_number'=>'','value'=>$value];
        }
        $gross=Money::zero(); foreach($notes as $note) $gross=$gross->add($note['value']);
        if($gross->minorAmount()<=0) throw new InvalidArgumentException('O valor bruto total deve ser maior que zero.');

        $taxes=[]; $totalWithheld=Money::zero();
        foreach(self::TAXES as $key=>$meta){
            $enabled=$input->{$meta['apply']}; $rate=Percentage::fromString($input->{$meta['rate']}); $basePercent=Percentage::fromString($input->{$meta['base']});
        ...
        if($totalWithheld->minorAmount()>$gross->minorAmount()) throw new InvalidArgumentException('Os parâmetros informados resultam em retenções superiores ao valor bruto. Revise alíquotas e bases.');
        ...
        foreach($notes as $note){
            ...
            foreach(self::TAXES as $key=>$meta){
                ...
                $row['taxes'][$key]=['base_minor'=>$base->minorAmount(),'withheld_minor'=>$ret->minorAmount()]; $row['withheld_minor'] += $ret->minorAmount();
            }
            $row['net_minor']=$row['gross_minor']-$row['withheld_minor']; $noteBreakdown[]=$row;
        }
