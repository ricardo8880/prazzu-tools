...
<x-tools.page title="Calculadora de Retenções na Nota Fiscal" description="Estime retenções da nota com parâmetros revisáveis e veja quanto fica retido e o líquido estimado." icon="bi-receipt-cutoff" slug="calculadora-retencoes-nota-fiscal">
    ...
    @auth
        <div class="mb-3"><a class="btn btn-sm btn-outline-secondary" href="{{ route('tools.calculadora-retencoes-nota-fiscal.history.index') }}"><i class="bi bi-clock-history me-1"></i>Histórico Plus</a></div>
    @endauth
    ...
    <form method="POST" action="{{ route('tools.calculadora-retencoes-nota-fiscal.calculate') }}" class="vstack gap-4">
        ...
        <x-tools.form-panel title="Nota ou serviço" description="O Essencial resolve uma nota individual e mostra cada retenção com memória de cálculo." badge="Essencial">
            <div class="row g-3 mb-4">
                <div class="col-md-3"><label class="form-label" for="competence">Competência</label><input class="form-control" type="month" id="competence" name="competence" min="2026-01" max="2026-12" value="{{ old('competence', now()->format('Y-m')) }}" required></div>
                ...
                <div class="col-md-6"><label class="form-label" for="service_description">Serviço / natureza da operação</label><input class="form-control" id="service_description" name="service_description" value="{{ old('service_description') }}" data-e2e-value="Serviço de consultoria" required placeholder="Ex.: Assessoria contábil"></div>
        ...
        <x-tools.form-panel title="Regras avançadas e conferência em lote" description="Ajuste a base de cada tributo e inclua outras notas/serviços com as mesmas regras." badge="Prazzu Plus">
            ...
            <div class="table-responsive"><table class="table table-sm align-middle"><thead><tr><th>Descrição</th><th>Valor bruto</th></tr></thead><tbody>
                @for($i=0;$i<5;$i++)
                    <tr><td><input class="form-control" name="notes[{{ $i }}][description]" value="{{ old("notes.$i.description") }}" placeholder="Ex.: Nota adicional {{ $i+1 }}"></td><td><input class="form-control" name="notes[{{ $i }}][value]" value="{{ old("notes.$i.value") }}" inputmode="decimal"></td></tr>
                @endfor
            </tbody></table></div>
