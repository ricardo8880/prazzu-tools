    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            foreach (['irrf', 'inss', 'iss', 'pis', 'cofins', 'csll'] as $tax) {
                if ($this->boolean('apply_'.$tax)) {
                    $enabled = true;
                    break;
                }
            }
            if (! $enabled) {
                $validator->errors()->add('apply_irrf', 'Selecione ao menos uma retenção para calcular.');
            }
            try {
                $total = Money::fromDecimal((string) $this->input('gross_value', '0'));
                foreach ((array) $this->input('notes', []) as $note) {
                    $v = trim((string) ($note['value'] ?? ''));
                    if ($v !== '') {
                        $total = $total->add(Money::fromDecimal($v));
                    }
                }
                if ($total->minorAmount() <= 0) {
                    $validator->errors()->add('gross_value', 'Informe valor bruto total maior que zero.');
                }
            } catch (Throwable) {
            }
        });
