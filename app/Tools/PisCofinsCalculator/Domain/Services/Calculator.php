...
final class Calculator implements ToolCalculator
{
    ...
    public function calculate(ToolCalculationInput $input): ToolCalculationResult
    {
        if (! $input instanceof CalculationInput) {
            throw new InvalidArgumentException('Entrada incompatível com a ferramenta calculadora-pis-cofins.');
        }
        if (! in_array($input->regime, ['cumulative', 'non_cumulative'], true)) {
            throw new InvalidArgumentException('Regime de apuração inválido.');
        }
        ...
        foreach ($input->operations as $operation) {
            ...
            if ($opRevenue->minorAmount() === 0 && $opCredit->minorAmount() === 0) {
                continue;
            }
            $revenue = $revenue->add($opRevenue);
            $creditBase = $creditBase->add($opCredit);
        ...
        if ($revenue->minorAmount() <= 0) {
            throw new InvalidArgumentException('A base tributável total deve ser maior que zero.');
        }

        $pisWithheld = Money::fromDecimal($input->pisWithheld);
        $cofinsWithheld = Money::fromDecimal($input->cofinsWithheld);
    ...
    private function calculateRegime(string $regime, Money $revenue, Money $creditBase, Money $pisWithheld, Money $cofinsWithheld): array
    {
        ...
        $pisDebit = $revenue->percentage($pisRate);
        $cofinsDebit = $revenue->percentage($cofinsRate);
        $pisCredit = $non ? $creditBase->percentage($pisRate) : Money::zero();
        $cofinsCredit = $non ? $creditBase->percentage($cofinsRate) : Money::zero();
        $pisBefore = max(0, $pisDebit->minorAmount() - $pisCredit->minorAmount());
        $cofinsBefore = max(0, $cofinsDebit->minorAmount() - $cofinsCredit->minorAmount());
        $pisPayable = max(0, $pisBefore - $pisWithheld->minorAmount());
        $cofinsPayable = max(0, $cofinsBefore - $cofinsWithheld->minorAmount());
