...
final readonly class IcmsInterstateRule implements EffectiveDated, NormativeRule
{
    ...
    public function rateFor(string $origin, string $destination, bool $imported): string
    {
        if ($imported) {
            return '4';
        } $southEastSouth = ['SP', 'RJ', 'MG', 'RS', 'SC', 'PR'];
        $sevenDest = ['AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'PA', 'PB', 'PE', 'PI', 'RN', 'RO', 'RR', 'SE', 'TO'];

        return in_array($origin, $southEastSouth, true) && in_array($destination, $sevenDest, true) ? '7' : '12';
    }
}
