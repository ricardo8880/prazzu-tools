...
{
    #[CoversPlusFeature('emissor-de-recibos', 'custom_branding')]
    public function test_lot_six_features_block_free_and_allow_plus_in_monetized_mode(): void
    {
        $manifest = (new Tool)->manifest();
        $this->assertFeatureAccess($manifest, 'custom_branding');
        $this->assertFeatureAccess($manifest, 'batch_generation');
        $this->assertFeatureAccess($manifest, 'history');
        $this->assertFeatureAccess($manifest, 'saved_profiles');
    }
