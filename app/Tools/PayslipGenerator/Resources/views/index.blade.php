...
<x-tools.form-panel title="Dados do holerite"><form method="POST" action="{{route('tools.gerador-holerite.calculate')}}" class="row g-3">@csrf
...
@isset($result)
    <span data-analytics-result="main" hidden></span>@php($d=$result->details['document'])<x-tools.result-panel title="Holerite"><div class="card"><div class="card-body"><div class="d-flex justify-content-between"><strong>{{$d['employer']}}</strong><span>Competência {{$d['competence']}}</span></div><p>{{$d['employee']}} — {{$d['document']}}</p><div class="table-responsive"><table class="table"><tbody>@foreach($d['items'] as $l=>$v)<tr><th>{{$l}}</th><td class="text-end">{{$v}}</td></tr>@endforeach</tbody><tfoot><tr><th>Líquido</th><th class="text-end">{{$d['net']}}</th></tr></tfoot></table></div></div></div><x-tools.export-buttons :pdf-route="route('tools.gerador-holerite.export.pdf')" :excel-route="route('tools.gerador-holerite.export.excel')" :input="$calculationInput ?? []" /></x-tools.result-panel>@endisset</x-tools.page>
@endsection
