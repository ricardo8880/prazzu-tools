...
@isset($result)<x-tools.result-panel title="Custo da admissão"><div class="row g-3">@foreach($result->summary as $item)<div class="col-md-6"><x-tools.result-metric :label="$item->label" :value="$item->value"/></div>@endforeach</div><h3 class="h5 mt-4">Checklist</h3><ul class="list-group">@foreach($result->details['checklist'] as $item)<li class="list-group-item"><input class="form-check-input me-2" type="checkbox"> {{$item}}</li>@endforeach</ul><x-tools.print-button class="mt-3">Imprimir / salvar PDF</x-tools.print-button></x-tools.result-panel>@endisset</x-tools.page>
@endsection
