...
{
    #[CoversPlusFeature('calculadora-turnover', 'segmented_analysis')]
    public function test_segmented_analysis_blocks_free_and_allows_plus_in_monetized_mode(): void
    {
        $manifest = (new Tool)->manifest();
        $user = $this->createStub(Authenticatable::class);
        $free = $this->gate(SubscriptionPlan::Free)->decide($manifest, 'segmented_analysis', $user);
        $plus = $this->gate(SubscriptionPlan::Plus)->decide($manifest, 'segmented_analysis', $user);
