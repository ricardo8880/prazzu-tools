...
{
    #[CoversPlusFeature('custo-funcionario-clt', 'branded_report')]
    #[CoversPlusFeature('custo-funcionario-clt', 'projections')]
    public function test_lot_six_features_block_free_and_allow_plus_in_monetized_mode(): void
    {
        $manifest = (new Tool)->manifest();
        $this->assertFeatureAccess($manifest, 'branded_report');
        $this->assertFeatureAccess($manifest, 'projections');
        $this->assertFeatureAccess($manifest, 'batch_processing');
        $this->assertFeatureAccess($manifest, 'company_profiles');
        $this->assertFeatureAccess($manifest, 'csv_export');
        $this->assertFeatureAccess($manifest, 'csv_import');
        $this->assertFeatureAccess($manifest, 'employee_profiles');
        $this->assertFeatureAccess($manifest, 'employment_model_comparison');
        $this->assertFeatureAccess($manifest, 'history');
        $this->assertFeatureAccess($manifest, 'professional_report');
        $this->assertFeatureAccess($manifest, 'scenarios');
        $this->assertFeatureAccess($manifest, 'xlsx_export');
        $this->assertFeatureAccess($manifest, 'xlsx_import');
    }
