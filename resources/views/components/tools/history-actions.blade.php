...
<div {{ $attributes->class(['d-inline-flex flex-wrap justify-content-end gap-1']) }}>
    ...
    @if ($deleteUrl)<form method="post" action="{{ $deleteUrl }}" data-confirm="Excluir este registro?">@csrf @method('DELETE')<button class="btn{{ $size }} btn-outline-danger" type="submit" aria-label="Excluir"><i class="bi bi-trash"></i></button></form>@endif
</div>
