    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-transparent d-flex flex-column flex-md-row justify-content-between gap-2">
            <div>
                ...
                <div class="small text-body-secondary">Feedback enviado pelos visitantes no período selecionado.</div>
            </div>
            <span class="badge text-bg-warning align-self-md-center"><i class="bi bi-star-fill me-1"></i>{{ number_format($feedback['average'], 2, ',', '.') }} de 5</span>
        </div>
        <div class="card-body">
            <div class="row g-3 mb-4">
                @foreach([
                    ['Avaliações recebidas', $feedback['total'], 'bi-chat-square-heart'],
                    ['Média geral', number_format($feedback['average'], 2, ',', '.').' / 5', 'bi-star'],
                    ['Notas 4 ou 5', number_format($feedback['positive_rate'], 1, ',', '.').'%', 'bi-hand-thumbs-up'],
                    ['Com comentários', $feedback['with_comment'], 'bi-chat-left-text'],
                ] as [$label, $value, $icon])
                        ...
                        <div class="border rounded-3 p-3 h-100">
                            <div class="d-flex justify-content-between gap-3">
                                <div>
                                    ...
                                    <div class="h4 mb-0 mt-1">{{ $value }}</div>
                                </div>
            ...
            <div class="row g-4">
                <div class="col-xl-4">
                    <h3 class="h6 mb-3">Distribuição das notas</h3>
                    @foreach($feedback['distribution'] as $rating => $total)
                        @php($percentage = $feedback['total'] > 0 ? ($total / $feedback['total']) * 100 : 0)
                        <div class="d-flex align-items-center gap-2 mb-2">
                            <span class="text-nowrap" style="width: 42px">{{ $rating }} <i class="bi bi-star-fill text-warning small"></i></span>
                            <div class="progress flex-grow-1" role="progressbar" aria-label="Nota {{ $rating }}" aria-valuenow="{{ round($percentage) }}" aria-valuemin="0" aria-valuemax="100" style="height: 10px">
                                <div class="progress-bar bg-warning" style="width: {{ $percentage }}%"></div>
                            </div>
                            <strong class="text-end" style="width: 34px">{{ $total }}</strong>
                        </div>
                    @endforeach
                </div>

                <div class="col-xl-8">
                    <h3 class="h6 mb-3">Páginas mais avaliadas</h3>
                    <div class="table-responsive border rounded-3">
                        <table class="table align-middle mb-0">
                            <thead><tr><th>Página</th><th class="text-end">Média</th><th class="text-end">Avaliações</th></tr></thead>
                            <tbody>
                            @forelse($feedback['top_pages'] as $page)
                                <tr>
                                    <td class="text-truncate" style="max-width: 420px" title="{{ $page->path }}">{{ $page->path }}</td>
                                    <td class="text-end"><span class="badge text-bg-warning"><i class="bi bi-star-fill me-1"></i>{{ number_format((float) $page->average_rating, 2, ',', '.') }}</span></td>
                                    <td class="text-end fw-semibold">{{ number_format($page->total, 0, ',', '.') }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="3" class="text-center text-body-secondary py-4">Nenhuma página foi avaliada neste período.</td></tr>
                            @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="card-header border-top bg-transparent fw-semibold">Comentários recentes</div>
        <div class="table-responsive">
            <table class="table align-middle mb-0">
                <thead><tr><th>Nota</th><th>Página</th><th>Comentário</th><th class="text-end">Data</th></tr></thead>
                <tbody>
                @forelse($feedback['recent'] as $item)
                    <tr>
                        <td class="text-nowrap"><span class="badge text-bg-warning">{{ $item->rating }} <i class="bi bi-star-fill"></i></span></td>
                        <td>
                            <div class="fw-semibold text-truncate" style="max-width: 240px">{{ $item->page_title ?: $item->path }}</div>
                            @if($item->page_title)<div class="small text-body-secondary text-truncate" style="max-width: 240px">{{ $item->path }}</div>@endif
                        </td>
                        <td style="min-width: 280px">{{ $item->comment ?: 'Sem comentário.' }}</td>
                        <td class="text-end text-nowrap">{{ $item->created_at->format('d/m/Y H:i') }}</td>
                    </tr>
                @empty
