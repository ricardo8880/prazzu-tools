...
<div class="container-fluid py-4">
    ...
    <div class="card border-0 shadow-sm mb-4"><div class="card-body d-flex flex-column flex-lg-row justify-content-between gap-3">
        ...
        @if($selected_funnel->custom)<form method="post" action="{{ route('admin.analytics.funnels.destroy', $selected_funnel->id) }}" data-confirm="Remover este funil?">@csrf @method('delete')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash me-1"></i>Remover</button></form>@endif
    </div></div>
...
@if($errors->any())<script nonce="{{ $cspNonce ?? '' }}">document.addEventListener('DOMContentLoaded',()=>bootstrap.Modal.getOrCreateInstance(document.getElementById('newFunnel')).show())</script>@endif
@endsection
